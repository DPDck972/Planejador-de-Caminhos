...
Ponto Planejador::getPonto(const IDPonto& Id) const
{
  ...
  if(Id.valid()){
    ...
    //Se o ponto não for igual a ponto.end retorna o ponto em questão
    if(itr != pontos.end()){
      return *itr;
    }
  }
...
Rota Planejador::getRota(const IDRota& Id) const
{
  ...
  if(Id.valid()){
    ...
    //Se as rotas não forem igual a rotas.end retorna o rota em questão
  ...
  try
  {
    ...
    do{
      ...
      if(atual.id_pt != id_destino){
        ...
        }while( find(this->rotas.begin(),this->rotas.end(),rota_suc) != this->rotas.end() );
      }
